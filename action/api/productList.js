...
class productList extends baseApi{
    async run(){
        await super.run();
        ...
        setup(paramProps);
        let categories = await enterpriseInstance.news_categories_son({id:3});
        if(param.cat===1 || categories.filter((item)=>{return item.id === param.cat}).length>0){
