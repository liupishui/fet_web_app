...
class imageList extends baseApi {
    async run() {
        await super.run();
        ...
        setup(paramProps);
        let categories = await enterpriseInstance.news_categories_son({ id: 5 });
        if (param.cat === 5 || categories.filter((item) => { return item.id === param.cat }).length > 0) {
            let data = await enterpriseInstance.image_list_server(param.cat, param.pageSize);
