...
class caseDetail extends baseApi{
    async run(){        
        await super.run();
        ...
        if(data){
            let catCase = await enterpriseInstance.news_categories_son({id:2});
            let isInCatCase = catCase.filter((item)=>{return item.id == data.category_id}).length>0;
            if(isInCatCase){
