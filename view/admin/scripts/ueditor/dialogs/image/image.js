...
(function () {
	...
	SearchImage.prototype = {
		...
		initEvents: function () {
            ...
            /* 滚动拉取图片 */
			// console.log(3333,$G('searchListUl'),domUtils);
            domUtils.on($G('searchListUl'), 'scroll', function(e){
                var panel = this;
                if (panel.scrollHeight - (panel.offsetHeight + panel.scrollTop) < 10) {
                    _this.page = Math.ceil(panel.children.length/30)+1;
					_this.getImageData();
                }
            });
			/* 点击搜索按钮 */
		...
		getImageData: function (offset) {
			...
			var _this = this,
				...
				url = "../../fet/imgsearch.ejs?pn="+ this.page*30 +"&word=" + type + key + "&keeporiginname=" + keepOriginName + "&" + +new Date;
				//console.log(_this);
			if (_this.isLoadingData===false) {
				...
				$.ajax({
					...
					success:function(data){
						...
						for (var i = 0; i < data.data.length; i++) {
							if (data.data[i].objURL) {
								list.push({
									title: data.data[i].fromPageTitleEnc,
									src: data.data[i].middleURL,
									url: data.data[i].replaceUrl[0].FromUrl
								});
							}
						}
						if (offset) {
		...
		setList: function (list) {
			...
			if (list.length) {
				...
				if (listUl.innerHTML=='') {
					...
					div.innerHTML = lang.searchRetry;
		...
		getInsertList: function () {
			...
			for (var i = 0; i < items.length; i++) {
				...
				if (child.tagName && child.tagName.toLowerCase() == 'img' && domUtils.hasClass(items[i], 'selected')) {
					src = child.src;
					var _from = items[i].querySelector('a').getAttribute('href');
					list.push({
						src: src,
						_src: src,
						_from: _from,
						alt:items[i].querySelector('a').innerText,
						//alt: src.substr(src.lastIndexOf('/') + 1),
						floatStyle: align
					});
				}
