...
function update($md5, $index)
{
  ...
  $files = new \core\model\files;
  return $files->update([':file_index'=>$index],"file_md5='{$md5}'");
}
...
// $re = $DB->fetch($sql);
$re = $files->get_one("file_md5 = '{$md5}'");
$id = 0;
if (isset($re)) {
  // 文件可访问的地址
  $id = $re['id'];
  $url =  $re['path'];
  $path = $_SERVER['DOCUMENT_ROOT'].'/'.$re['path'];
  ...
  if ($file_index <= $file_total) {
    ...
    $appendRst = file_put_contents($path, $content, FILE_APPEND);
    if (!$appendRst) {
      jsonMsg(0, '无法写入文件');
    }
    update($re['file_md5'],$index);
    // 片数相等，等于完成了
    if ($index == $total) {
      $getHtml = new \core\lib\gethtml;
      $getHtml->imgConvert($path,pathinfo($url,PATHINFO_DIRNAME),false,false,true,true);
      jsonMsg(2, '上传完成', $url, $index,$name);
    }
